...
Classifier가 생성하는 query는 **완전한 문장**이어야 함. Worker Agent(Haiku)가 이 query만 보고 어떤 MCP 도구를 호출할지 결정하므로, 의도가 명확해야 함.

- BAD: `"fluss Confluence"`, `"데이터 스키마"` (키워드만 — Worker가 도구 선택 못함)
...
`confidence`는 `null`/`NaN` 가능 — `Number.isFinite()` 가드 적용 (agent-steps, chat-workflow .map(), quality-check).
...
clarify suspend 시 `suspendMeta`(runId, suspendedStep, hitlType)를 `chats.suspend_meta` 컬럼에 DB 저장.
스레드 전환(Chat 컴포넌트 unmount/remount) 시에도 resume 가능.

- `/chat` 핸들러: clarify suspend 시 DB 저장 → resume/completion 시 `null`로 클리어
- `chat/[id]/page.tsx`: DB에서 `initialSuspendMeta`를 읽어 Chat 컴포넌트에 전달
- `chat.tsx`: `suspendMetaRef`를 `initialSuspendMeta`로 초기화 (기존: `null`)

### Chat UI 연동 (`src/components/chat.tsx`)

AI SDK `useChat` + `DefaultChatTransport` 기반:
- `suspendMetaRef` — clarify suspend 메타데이터 관리 (useRef, DB에서 복원 가능)
